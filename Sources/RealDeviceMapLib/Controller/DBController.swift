...
public class DBController {
    ...
    private func backup(mysql: MySQL, fromVersion: Int, toVersion: Int) {
        ...
        if fromVersion < toVersion && backupEnabled {
            ...
            let commandSchema = Shell("bash", "-c", mysqldumpCommand + " --set-gtid-purged=OFF --skip-triggers --add-drop-table --skip-routines --no-data \(self.database) \(tablesSchema) -h \(self.host) -P \(self.port) -u \(self.rootUsername) -p\(self.rootPassword.stringByReplacing(string: "\"", withString: "\\\"")) > \(backupFileSchema.path)")
            ...
            // Routines
            //  swiftlint:disable:next line_length
            let commandRoutine = Shell("bash", "-c", mysqldumpCommand + " --set-gtid-purged=OFF --skip-triggers --no-create-info --no-data --routines \(self.database) -h \(self.host) -P \(self.port) -u \(self.rootUsername) -p\(self.rootPassword.stringByReplacing(string: "\"", withString: "\\\"")) > \(backupFileRoutine.path)")
            ...
            // Trigger
            //  swiftlint:disable:next line_length
            let commandTrigger = Shell("bash", "-c", mysqldumpCommand + " --set-gtid-purged=OFF --triggers --no-create-info --no-data --skip-routines \(self.database) \(tablesSchema)  -h \(self.host) -P \(self.port) -u \(self.rootUsername) -p\(self.rootPassword.stringByReplacing(string: "\"", withString: "\\\"")) > \(backupFileTrigger.path)")
